...
SAVE_ROOT=${7:-${EVAL_SAVE_ROOT:-"/media/drive2/distillation/safety-utility-pg/results/qwen3/eval_main"}}
...
if [ "$PEFT_CKPT_NAME" == "Qwen2.5-3B-Instruct" -o "$PEFT_CKPT_NAME" == "Qwen2.5-7B-Instruct" ]
then
    PEFT_CKPT_NAME="test"
    PEFT_CKPT_PATH="test"
fi
...
do
    ...
    do
        ...
        bash "${base_path}/scripts/qwen2/eval/eval_main_${data}.sh" "$seed" "$PEFT_CKPT_NAME" "$PEFT_CKPT_PATH" "$CKPT_NAME" "$CKPT_PATH" "$GPUS_PER_NODE"
    done
done
