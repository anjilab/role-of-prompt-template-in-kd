...
def prepare_dataset_main(args, tokenizer: PreTrainedTokenizerFast | PreTrainedTokenizer):
    data = {}
    data["test"] = PromptDataset(args, tokenizer, "test", args.data_dir, args.dev_num)
...
def run_model(args, tokenizer: PreTrainedTokenizerFast | PreTrainedTokenizer, model, dataset: PromptDataset, epoch, device):
    ...
    generation_config = GenerationConfig (
        ...
        # max_length=args.max_length, # max_length is ignored, and max_new_tokens is used.
        min_length=None,
    ...
    with torch.no_grad():
        for it, (model_batch, no_model_batch) in enumerate(tqdm(dataloader, desc=f"Evaluating {args.data_type} ", disable=(dist.get_rank() != 0))):
            if it == 0:
                print_rank("############### Example ###############")
...
def evaluate_main(args, tokenizer: PreTrainedTokenizer | PreTrainedTokenizerFast, 
    ...
    if dist.get_rank() == 0:
        wandb.init(
            project='minillm-kd-evaluation',
            name=f"{args.data_type}_evaluation",
            config=vars(args)
        )
    ...
    print_rank(f"Data Type: {args.data_type}")
    
    if args.data_type == "meta-math":
        gen_res = compute_math_metrics(all_responses, dataset.answers)
    elif args.data_type == "med-qa":
        gen_res = compute_medqa_metrics(all_responses, dataset.answers)
    else:
        gen_res = compute_rouge_metrics(all_responses, dataset.answers)
