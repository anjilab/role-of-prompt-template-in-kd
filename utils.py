...
def get_model_bits_and_bytes_config(args, device: int) -> PreTrainedModel:
    config = AutoConfig.from_pretrained(args.model_path)
...
def get_tokenizer(args) -> PreTrainedTokenizer | PreTrainedTokenizerFast:
    ...
    if args.model_type in ["gpt2", "opt", "llama", "gptj", "llama2", "mistral", "qwen2","gemma","gemma2", "llama3"]:
...
def load_parallel(model: nn.Module, load_dir: str):
    ...
    return "Model Parallel Load is not implemented yet."
...
def save_parallel(model: nn.Module, save_dir: str):
    ...
    return "Model Parallel save is not implemented yet."
