...
from medqa_metric import compute_metrics as compute_medqa_metrics
from code_metric import compute_pass_at_1 as compute_code_pass_at_1
...
def collect_teacher_responses(args, tokenizer, teacher_model, gen_data):
    max_new_tokens = int(os.getenv("TEACHER_LOG_MAX_NEW_TOKENS", "128"))
    ...
    response_ids = out.sequences[:, prompt_len:]
...
def format_teacher_generation_log(args, tokenizer, teacher_model, gen_data, global_step):
    prompts, responses = collect_teacher_responses(args, tokenizer, teacher_model, gen_data)

    lines = [f"teacher_generations | global_step: {global_step}"]
    for i, (prompt, response) in enumerate(zip(prompts, responses), start=1):
        prompt = prompt.replace(chr(10), "<n>")
        response = response.replace(chr(10), "<n>")
        lines.append(f"[sample {i}] prompt: {prompt} | teacher_response: {response}")
    return "\n".join(lines)
...
def get_teacher_model(args, device: int):
    ...
    if args.model_parallel:
        ...
        # Check this refactored logic
        # if args.model_type in ['gemma2', 'llama3']:
        #     if args.type != "fkl+rkl":
        #         # Logic for gemma2 or llama3 when TYPE is NOT "fkl+rkl"
        #         print("Case 1: gemma2 or llama3, TYPE != fkl+rkl")
        ...
        #             # Logic for llama3 when TYPE is "fkl+rkl"
        #             print("Case 2: llama3, TYPE == fkl+rkl")
        #             bnb_config = BitsAndBytesConfig(
        #                 load_in_8bit=True,        # or load_in_4bit=True if you want more savings
        #                 llm_int8_threshold=6.0,
        #                 )
        ...
        #             # Logic for gemma2 when TYPE is "fkl+rkl"
        #             print("Case 3: gemma2, TYPE == fkl+rkl")
        #             bnb_config = BitsAndBytesConfig(
        #                     load_in_4bit=True,        # or load_in_4bit=True if you want more savings
        #                     # llm_int8_threshold=6.0,
        ...
        if args.model_type == 'gemma2' or args.model_type == 'llama3' or args.model_type == 'qwen2':
            bnb_config = BitsAndBytesConfig(
                load_in_8bit=True,        # or load_in_4bit=True if you want more savings
                llm_int8_threshold=6.0,
            )
        ...
        # # this is to be commented out while running llama models.
        # # if args.model_type == 'gemma2' and args.type == "fkl+rkl": # For fkl + rkl need to do this.
        # if args.model_type == 'gemma2' and args.type == "jsd": # For fkl + rkl need to do this.
...
def get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits):
    with torch.no_grad():
        ...
        if args.model_type == 'qwen2':
            shared_vocab_size = min(logits.size(-1), teacher_logits.size(-1))
    ...
    if args.model_parallel:
        ...
        if args.type == "rkl":
        ...
        elif args.type == "fkl+rkl":
            inf_mask = torch.isinf(logits)  # [B, L, V]
            mask = (no_model_batch["label"] != -100).int()  # [B, L]
            fkl_loss = get_kl(teacher_logits, logits, inf_mask, mask)
            rkl_loss = get_kl(logits, teacher_logits, inf_mask, mask)
            distil_loss = 0.5 * (fkl_loss + rkl_loss)
            ...
            # print_rank("Using standard distillation loss")
            teacher_probs = F.softmax(teacher_logits, dim=-1, dtype=torch.float32)
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if teacher_model is not None:
                ...
                # KD warmup: ramp from 0 to kd_ratio over first epoch
                warmup_steps = args.train_iters_per_epoch  # 1 epoch warmup
                current_kd_ratio = min(args.kd_ratio, args.kd_ratio * (global_step / warmup_steps))
                
                loss = (1 - current_kd_ratio) * lm_loss + current_kd_ratio * distil_loss
            ...
            if global_step % args.log_interval == 0 and step % args.gradient_accumulation_steps == 0:
                ...
                if teacher_model is not None and dist.get_rank() == 0:
                    teacher_log_str = format_teacher_generation_log(
                        args, tokenizer, teacher_model, gen_data, global_step
                    )
                    save_rank(teacher_log_str, os.path.join(args.save, "teacher_log.txt"))
                if dist.get_rank() == 0:
                    wandb.log({
                        ...
                        # "kd_ratio": current_kd_ratio,  # add this
                    })
                total_loss, total_distil_loss, total_time = 0.0, 0.0, 0.0
...
def evaluate(args, tokenizer: PreTrainedTokenizer | PreTrainedTokenizerFast, model: PreTrainedModel, 
    ...
    if get_rank() == 0:
        if args.eval_gen:
            ...
            elif args.data_type == "med-qa":
                res = compute_medqa_metrics(responses, references)
            elif args.data_type in {"magicoder-python-clean", "code-alpaca", "coding"}:
                if any(dataset.test_cases):
                    res = compute_code_pass_at_1(responses, dataset.test_cases)
                else:
...
def main():
    ...
    if dist.get_rank() == 0:
        wandb.init(
            # project="distillation_safety_utility_dolly_without_it_student_models",
            project="distillation_safety_utility_utility_metamath",
