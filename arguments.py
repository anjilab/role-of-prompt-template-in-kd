...
def add_model_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--dtype", type=str, choices=["torch.float32", "torch.float16", "torch.bfloat16"], default="torch.bfloat16")
    return parser
...
def add_runtime_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--type-variant", type=str, default=None)
...
def add_data_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--data-type", type=str, default="dolly")
    ...
    group.add_argument("--curriculum-mix-ratio", type=float, default=None,
                       help='Curriculum learning mix ratio for prompt types (0.0-1.0). E.g., 0.5 means 50% of curriculum type, 50% default.')
    return parser
...
def add_hp_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--wandb-project-name", type=str, default="distillation_safety_utility_metamath")
    # group.add_argument("--wandb-project-name", type=str, default="distillation_safety_utility_prompts_variation")
    
    group.add_argument("--wandb-run-name", type=str, default="distillation_safety_utility_metamath")
    # group.add_argument("--wandb-run-name", type=str, default="distillation_safety_utility_prompts_variation")
...
def get_args():
    ...
    if args.type == "eval_main":
        ...
        args.save = save_path
    elif args.type == "kd" or args.type == "rkl":
        save_path = os.path.join(
            args.save,
            (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else "" + \
             f"teacher-base/{args.teacher_ckpt_name}" + f"/teacher-adapter/{args.teacher_peft_name}" if args.teacher_peft_name is not None else ""),
    ...
    # Ensure `args.save` is unique per run by appending a timestamp subdirectory when provided.
    if args.save is not None:
        ts = time.strftime("%Y%m%d_%H%M%S", time.localtime())
        args.save = os.path.join(args.save, ts)
